...
static PyObject *init(PyObject *self, PyObject *args) {
    ...
    if (PyArg_ParseTuple(args, "|ss:init", &cec_name, &dev)) {
        debug("%s\n", dev);
        debug("%s\n", cec_name);
        if (strcmp(cec_name, "") == 0){
            cec_name = "python_cec";
        }
        if (!dev) {
...
void activated_cb(void *self, const cec_logical_address logical_address, const uint8_t state) {
...
PyMODINIT_FUNC initcec(void) {
    ...
    debug("%s\n", cec_name);
    snprintf(CEC_config->strDeviceName, 13, "%s", cec_name);
